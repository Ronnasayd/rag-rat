...
/// `rag_rat_base::path_class::file_is_generated`: explicit `kind = generated` OR the codegen path
/// heuristic, so it also catches generated code living under a *source* target). Only the *search*
/// arms (name, symbol_path) apply it; an explicit `symbol_id` / `logical_symbol_id` selection is a
/// deliberate pick and is never filtered.
...
pub const SYMBOL_KIND_RANK: &[(&str, i64)] = &[
